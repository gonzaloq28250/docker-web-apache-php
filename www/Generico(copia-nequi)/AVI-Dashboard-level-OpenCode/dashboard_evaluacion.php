...
$fechaDesde = !empty($_GET['fecha_desde']) ? $_GET['fecha_desde'] : date('Y-m-d', strtotime('-30 days'));
...
$stmtTotal = $pdo->prepare("
    ...
    AND DATE(fecha_evaluacion) BETWEEN :desde AND :hasta
");
...
$stmtPendientes = $pdo->prepare("
    ...
    AND DATE(lc.F9TimeStamp) BETWEEN :desde AND :hasta
    AND NOT EXISTS (
...
$stmtMatriz = $pdo->prepare("
    ...
    AND DATE(e.fecha_evaluacion) BETWEEN :desde AND :hasta
...
$whereUltimas = "e.cliente = :cliente AND DATE(e.fecha_evaluacion) BETWEEN :desde AND :hasta";
...
$stmtUltimas = $pdo->prepare("
    SELECT e.F9CallID, e.ANI, e.resultado, e.call_result_correcto, e.se_puede_mejorar, e.info_disponible_sa, e.observacion, e.fecha_evaluacion,
           lc.duration, lc.F9TimeStamp, lvc.call_result as original_result
    FROM level_transcripciones_evaluacion e
