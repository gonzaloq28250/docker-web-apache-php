...
$clientesDisponibles = ['NEQUI', 'NEQUI2'];
...
$fechaDesde = !empty($_GET['fecha_desde']) ? $_GET['fecha_desde'] : date('Y-m-d', strtotime('-7 days')) . 'T00:00';
...
$where = "WHERE lc.cliente = :cliente";
$params = [':cliente' => $clienteActual];

if (!empty($fechaDesdeSQL) && !empty($fechaHastaSQL)) {
    $where .= " AND lc.F9TimeStamp BETWEEN :fecha_desde AND :fecha_hasta";
    $params[':fecha_desde'] = $fechaDesdeSQL;
    $params[':fecha_hasta'] = $fechaHastaSQL;
} elseif (!empty($fechaDesdeSQL)) {
    $where .= " AND lc.F9TimeStamp >= :fecha_desde";
    $params[':fecha_desde'] = $fechaDesdeSQL;
} elseif (!empty($fechaHastaSQL)) {
    $where .= " AND lc.F9TimeStamp <= :fecha_hasta";
    $params[':fecha_hasta'] = $fechaHastaSQL;
}

if (!empty($buscar)) {
    $where .= " AND (lc.F9CallID LIKE :buscar OR lc.ANI LIKE :buscar2 OR lc.DNIS LIKE :buscar3)";
        ...
        <div class="max-w-7xl mx-auto px-4 py-6">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="dashboard_consolidado.php<?= $clienteActual !== CLIENTE_ACTUAL ? '?cliente='.$clienteActual : '' ?>" class="bg-teal-500 hover:bg-teal-400 text-white px-4 py-2 rounded-lg transition flex items-center gap-2 text-sm">
                        <i class="fas fa-chart-pie"></i> Dashboard Consolidado
                    </a>
                </div>
            </div>
        </div>
                ...
                <div id="panel-evaluacion" class="hidden">
                    ...
                    <div class="bg-white rounded-lg shadow-md overflow-hidden">
                        ...
                        <div class="p-6">
                            ...
                            <!-- Selector de call_result correcto (solo visible cuando No Pasa) -->
