...
$stmtJoinedHoy = $pdo->prepare("
    ...
    ORDER BY total DESC
");
$stmtJoinedHoy->execute([':cliente' => $clienteActual, ':cliente2' => $clienteActual, ':fecha_hoy_ini' => $fechaHoyIni, ':fecha_hoy_fin' => $fechaHoyFin]);
...
if ($aplicarFiltros) {
    ...
    $whereAgg = "WHERE lc.cliente = :cliente AND lvc.customer = :cliente2";
    ...
    $whereLeads = "WHERE lc.cliente = :cliente";
...
<body class="bg-gray-50 min-h-screen">
    ...
    <header class="bg-gradient-to-r from-indigo-600 to-purple-700 text-white shadow-lg">
        <div class="max-w-7xl mx-auto px-4 py-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-3xl font-bold">Dashboard Consolidado <span class="text-indigo-200 text-xl font-normal">- <?= htmlspecialchars($clienteLabel) ?></span></h1>
    ...
    <script>
        ...
        function openDetailModal(f9CallID) {
            ...
            iframe.src = 'detalle_lead_v2.php?F9CallID=' + encodeURIComponent(f9CallID) + '&cliente=' + encodeURIComponent('<?= $clienteActual ?>');
        ...
        function exportResumenHistoricoToExcel() {
            ...
            // Encabezados
            const headers = ['Fecha', 'Llamadas Totales', '% Retención'];
            <?php foreach ($todosResultadosHistorico as $resultado): ?>
            headers.push('<?= addslashes($resultado) ?>');
            headers.push('%');
            <?php endforeach; ?>
            data.push(headers);
            ...
            ?>
            {
                const row = ['<?= date('d/m/Y', strtotime($fecha)) ?>', <?= $total ?>, <?= $total > 0 ? ($porcentajeRetencion / 100) : 0 ?>];
                <?php foreach ($todosResultadosHistorico as $resultado): ?>
                row.push(<?= $resultados[$resultado] ?? 0 ?>);
                row.push(<?= $total > 0 ? (($resultados[$resultado] ?? 0) / $total) : 0 ?>);
                <?php endforeach; ?>
                data.push(row);
            }
            <?php endforeach; ?>
            ...
            // Fila de totales
            {
                <?php
                $retencionTotal = ($totalesPorResultadoHistorico['contencion_exitosa'] ?? 0) + ($totalesPorResultadoHistorico['consulta_resuelta'] ?? 0);
                $retencionTotalPct = $totalGeneral > 0 ? $retencionTotal / $totalGeneral : 0;
                ?>
                const totalRow = ['TOTAL', <?= $totalGeneral ?>, <?= $retencionTotalPct ?>];
                <?php foreach ($todosResultadosHistorico as $resultado):
                    $pct = $totalGeneral > 0 ? (($totalesPorResultadoHistorico[$resultado] ?? 0) / $totalGeneral) : 0;
                ?>
                totalRow.push(<?= $totalesPorResultadoHistorico[$resultado] ?? 0 ?>);
                totalRow.push(<?= $pct ?>);
                <?php endforeach; ?>
                data.push(totalRow);
            }
            ...
            for (let C = 0; C <= range.e.c; C++) {
                // Columna 0: forzar formato texto para evitar que Excel convierta fechas
                if (C === 0) {
