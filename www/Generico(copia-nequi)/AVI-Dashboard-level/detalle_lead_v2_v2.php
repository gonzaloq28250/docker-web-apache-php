...
$backLink = 'dashboard_consolidado_v2.php' . (!empty($queryString) ? '?' . $queryString : '');

// Obtener datos de level_calls
$stmt_level = $pdo->prepare("SELECT DNIS, duration, ANI FROM level_calls WHERE F9CallID = :f9id AND cliente = :cliente LIMIT 1");
$stmt_level->execute([':f9id' => $f9id, ':cliente' => $clienteActual]);
...
$stmt_level_conv = $pdo->prepare("SELECT callid, transcript_summary, call_result, transcript, transcript_text, created_at, levelConversationID FROM level_conversations WHERE callid = :f9id AND customer = :cliente LIMIT 1");
$stmt_level_conv->execute([':f9id' => $f9id, ':cliente' => $clienteActual]);
