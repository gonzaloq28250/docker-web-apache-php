...
// Cliente seleccionable por URL (?cliente=...), validado contra el catálogo; si no, el predeterminado
$clientesDisponibles = CLIENTES_DISPONIBLES;            // etiqueta visible => valor en BD
...
// Gráfico Pie: Tipo de llamadas (hoy) - desde level_conversations
...
$stmtResultadosHoy = $pdo->prepare("
    ...
    ORDER BY total DESC
");
$stmtResultadosHoy->execute([':cliente' => $clienteActual, ':cliente2' => $clienteActual, ':fecha_hoy' => $fechaHoy]);
...
$colgadasHoy = 0; // No se calculan llamadas colgadas en REALTIME
...
$stmtDuracionTotalHoy = $pdo->prepare("
    ...
    AND lc.duration >= 60
");
$stmtDuracionTotalHoy->execute([':cliente' => $clienteActual, ':cliente2' => $clienteActual, ':fecha_hoy' => $fechaHoy]);
...
$paramsHistorico = [':cliente' => $clienteActual, ':cliente2' => $clienteActual];

// Filtro de rango de fechas con hora
if (!empty($fechaDesdeSQL) && !empty($fechaHastaSQL)) {
    $whereHistorico .= " AND F9TimeStamp BETWEEN :fecha_desde AND :fecha_hasta";
    $paramsHistorico[':fecha_desde'] = $fechaDesdeSQL;
    $paramsHistorico[':fecha_hasta'] = $fechaHastaSQL;
} elseif (!empty($fechaDesdeSQL)) {
    $whereHistorico .= " AND F9TimeStamp >= :fecha_desde";
    $paramsHistorico[':fecha_desde'] = $fechaDesdeSQL;
} elseif (!empty($fechaHastaSQL)) {
    $whereHistorico .= " AND F9TimeStamp <= :fecha_hasta";
    $paramsHistorico[':fecha_hasta'] = $fechaHastaSQL;
}
...
if ($aplicarFiltros) {
    // Gráfico Pie: Tipo de llamadas (filtrado) - Excluir IVR_Regular
    ...
    $stmtResultadosHistorico->execute($paramsHistorico);
    ...
    $colgadasHistorico = 0; // No se calculan llamadas colgadas en HISTÓRICO

    // KPI: Duración Total (filtrado) - usando level_calls.duration
    // Solo para llamadas con call_result (excluyendo IVR_Regular)
    $sqlDuracionTotalHistorico = "
        SELECT SUM(lc.duration) as total
        FROM level_calls lc
        LEFT JOIN level_conversations lvc ON lc.F9CallID = lvc.callid AND lvc.customer = :cliente2
        $whereHistorico
    ...
    $whereResumenDiario = "WHERE lc.cliente = :cliente";
    $paramsResumenDiario = [':cliente' => $clienteActual, ':cliente2' => $clienteActual];

    if (!empty($fechaDesdeSQL) && !empty($fechaHastaSQL)) {
        $whereResumenDiario .= " AND lc.F9TimeStamp BETWEEN :fecha_desde AND :fecha_hasta";
        $paramsResumenDiario[':fecha_desde'] = $fechaDesdeSQL;
        $paramsResumenDiario[':fecha_hasta'] = $fechaHastaSQL;
    } elseif (!empty($fechaDesdeSQL)) {
        $whereResumenDiario .= " AND lc.F9TimeStamp >= :fecha_desde";
        $paramsResumenDiario[':fecha_desde'] = $fechaDesdeSQL;
    } elseif (!empty($fechaHastaSQL)) {
        $whereResumenDiario .= " AND lc.F9TimeStamp <= :fecha_hasta";
        $paramsResumenDiario[':fecha_hasta'] = $fechaHastaSQL;
    }
    ...
    $totalPorFecha = []; // Calcular total por fecha desde los resultados
    ...
    // No se calculan llamadas colgadas en HISTÓRICO
    ...
    $whereLeadsHistorico = "WHERE cliente = :cliente";
    $paramsLeadsHistorico = [':cliente' => $clienteActual];

    if (!empty($fechaDesdeSQL) && !empty($fechaHastaSQL)) {
        $whereLeadsHistorico .= " AND F9TimeStamp BETWEEN :fecha_desde AND :fecha_hasta";
        $paramsLeadsHistorico[':fecha_desde'] = $fechaDesdeSQL;
        $paramsLeadsHistorico[':fecha_hasta'] = $fechaHastaSQL;
    } elseif (!empty($fechaDesdeSQL)) {
        $whereLeadsHistorico .= " AND F9TimeStamp >= :fecha_desde";
        $paramsLeadsHistorico[':fecha_desde'] = $fechaDesdeSQL;
    } elseif (!empty($fechaHastaSQL)) {
        $whereLeadsHistorico .= " AND F9TimeStamp <= :fecha_hasta";
        $paramsLeadsHistorico[':fecha_hasta'] = $fechaHastaSQL;
    }

    // Filtro de búsqueda
    if (!empty($buscar)) {
        $whereLeadsHistorico .= " AND (
            ANI LIKE :buscar OR
            DNIS LIKE :buscar OR
            PROYECTO LIKE :buscar OR
            F9CallID LIKE :buscar
        )";
        $paramsLeadsHistorico[':buscar'] = "%$buscar%";
    }

    $sqlLeadsHistorico = "SELECT F9CallID, F9TimeStamp, ANI, DNIS, PROYECTO, Estado, duration FROM level_calls $whereLeadsHistorico ORDER BY F9TimeStamp DESC";
    ...
    foreach ($rowsLeadsHistorico as $row) {
        $leadsHistorico[$row['F9CallID']] = $row;
    }

    // Obtener call_result de level_conversations
    $sqlResultadoHistorico = "
        SELECT lc.F9CallID, lvc.call_result as resultado_llamada
        FROM level_calls lc
        LEFT JOIN level_conversations lvc ON lc.F9CallID = lvc.callid AND lvc.customer = :cliente2
        WHERE lc.cliente = :cliente
    ";
    ...
    if (!empty($fechaDesdeSQL) && !empty($fechaHastaSQL)) {
        $sqlResultadoHistorico .= " AND lc.F9TimeStamp BETWEEN :fecha_desde AND :fecha_hasta";
    } elseif (!empty($fechaDesdeSQL)) {
        $sqlResultadoHistorico .= " AND lc.F9TimeStamp >= :fecha_desde";
    } elseif (!empty($fechaHastaSQL)) {
        $sqlResultadoHistorico .= " AND lc.F9TimeStamp <= :fecha_hasta";
    }

    $stmtResultadoHistorico = $pdo->prepare($sqlResultadoHistorico);
    $paramsResultado = [':cliente' => $clienteActual, ':cliente2' => $clienteActual];
    if (!empty($fechaDesdeSQL)) {
        $paramsResultado[':fecha_desde'] = $fechaDesdeSQL;
    }
    if (!empty($fechaHastaSQL)) {
        $paramsResultado[':fecha_hasta'] = $fechaHastaSQL;
    }
    $stmtResultadoHistorico->execute($paramsResultado);
    $rowsResultadoHistorico = $stmtResultadoHistorico->fetchAll(PDO::FETCH_ASSOC);

    // Organizar datos adicionales por F9CallID
    $datosAdicionalesPorF9CallID = [];
    foreach ($rowsResultadoHistorico as $row) {
        $datosAdicionalesPorF9CallID[$row['F9CallID']] = $row['resultado_llamada'] ?? '-';
    }
}
...
<head>
    ...
    <title>Dashboard Consolidado - <?= htmlspecialchars($clienteLabel) ?></title>
...
<body class="bg-gray-50 min-h-screen">
        ...
        <div class="max-w-7xl mx-auto px-4 py-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <form method="GET" class="mt-1 flex items-center gap-2">
                        <label for="cliente" class="text-indigo-100 text-sm">Cliente:</label>
                        ...
                        <input type="hidden" name="fecha_desde" value="<?= htmlspecialchars($_GET['fecha_desde'] ?? '') ?>">
                        <input type="hidden" name="fecha_hasta" value="<?= htmlspecialchars($_GET['fecha_hasta'] ?? '') ?>">
            ...
            <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                ...
                <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="md:col-span-3 flex gap-3">
                        ...
                        <a href="dashboard_consolidado.php?cliente=<?= urlencode($clienteActual) ?>" class="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-3 rounded-lg transition duration-200 flex items-center justify-center gap-2 text-center">
    ...
    <script>
        ...
        function refreshRealtimeData() {
            fetch('dashboard_realtime_data.php?cliente=<?= urlencode($clienteActual) ?>')
