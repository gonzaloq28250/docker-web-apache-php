...
// KPI 4: LLM Costo Total (hoy)
$stmtLLMCostoHoy = $pdo->prepare("
    SELECT SUM(llm_cost_total_usd) as total
    FROM avi_call_costs
    WHERE cliente = :cliente
    AND DATE(metadata_date_local) = :fecha_hoy
");
$stmtLLMCostoHoy->execute([':cliente' => $clienteActual, ':fecha_hoy' => $fechaHoy]);
$llmCostoTotalHoy = $stmtLLMCostoHoy->fetch()['total'] ?? 0;

// KPI 5: LLM Costo Promedio (hoy)
$llmCostoPromedioHoy = $totalLlamadasAVIHoy > 0 ? $llmCostoTotalHoy / $totalLlamadasAVIHoy : 0;
...
// KPI 6: Tasa Retención AVI (hoy)
// Cuenta con "contencion_exitosa", "consulta_resuelta" y "consulta_resuelta_pase_asesor"
$retencionExitosaHoy = 0;
$consultaResueltaHoy = 0;
$consultaResueltaPaseAsesorHoy = 0;
foreach ($resultadosHoy as $row) {
    ...
    } elseif ($row['resultado'] === 'consulta_resuelta_pase_asesor') {
        $consultaResueltaPaseAsesorHoy = $row['total'];
    }
}
$tasaRetencionAVIHoy = $totalLlamadasAVIHoy > 0 ? (($retencionExitosaHoy + $consultaResueltaHoy + $consultaResueltaPaseAsesorHoy) / $totalLlamadasAVIHoy) * 100 : 0;
...
if ($aplicarFiltros) {
    ...
    // KPI 4: LLM Costo Total (filtrado)
    $sqlLLMCostoHistorico = "SELECT SUM(llm_cost_total_usd) as total FROM avi_call_costs $whereHistorico";
    $stmtLLMCostoHistorico = $pdo->prepare($sqlLLMCostoHistorico);
    $stmtLLMCostoHistorico->execute($paramsHistorico);
    $llmCostoTotalHistorico = $stmtLLMCostoHistorico->fetch()['total'] ?? 0;

    // KPI 5: LLM Costo Promedio (filtrado)
    $llmCostoPromedioHistorico = $totalLlamadasAVIHistorico > 0 ? $llmCostoTotalHistorico / $totalLlamadasAVIHistorico : 0;
    ...
    // KPI 6: Tasa Retención AVI (filtrado)
    // Cuenta con "contencion_exitosa", "consulta_resuelta" y "consulta_resuelta_pase_asesor"
    $retencionExitosaHistorico = 0;
    $consultaResueltaHistorico = 0;
    $consultaResueltaPaseAsesorHistorico = 0;
    foreach ($resultadosHistorico as $row) {
        if ($row['resultado'] === 'contencion_exitosa') {
            $retencionExitosaHistorico = $row['total'];
        } elseif ($row['resultado'] === 'consulta_resuelta') {
            $consultaResueltaHistorico = $row['total'];
        } elseif ($row['resultado'] === 'consulta_resuelta_pase_asesor') {
            $consultaResueltaPaseAsesorHistorico = $row['total'];
        }
    }
    $tasaRetencionAVIHistorico = $totalLlamadasAVIHistorico > 0 ? (($retencionExitosaHistorico + $consultaResueltaHistorico + $consultaResueltaPaseAsesorHistorico) / $totalLlamadasAVIHistorico) * 100 : 0;
    ...
    // Obtener datos adicionales de ResultadoPerfil (resultado_llamada, etc)
    $sqlResultadoPerfilHistorico = "
        SELECT r.F9CallID, r.clave, r.valor
        FROM siigo_lead_data_v2 r
        WHERE r.method = 'ResultadoPerfil'
        AND EXISTS (
            SELECT 1 FROM siigo_lead_data_v2 c
            WHERE c.F9CallID = r.F9CallID AND c.clave = 'CLIENTE' AND c.valor = :cliente
        )
    ";

    // Agregar filtros de fecha si existen
    if (!empty($fechaDesdeSQL) && !empty($fechaHastaSQL)) {
        $sqlResultadoPerfilHistorico .= " AND EXISTS (
            SELECT 1 FROM siigo_lead_data_v2 ts
            WHERE ts.F9CallID = r.F9CallID AND ts.clave = 'F9TimeStamp'
            AND ts.valor BETWEEN :fecha_desde AND :fecha_hasta
        )";
    }

    $stmtResultadoPerfilHistorico = $pdo->prepare($sqlResultadoPerfilHistorico);
    $paramsResultadoPerfil = [':cliente' => $clienteActual];
    if (!empty($fechaDesdeSQL) && !empty($fechaHastaSQL)) {
        $paramsResultadoPerfil[':fecha_desde'] = $fechaDesdeSQL;
        $paramsResultadoPerfil[':fecha_hasta'] = $fechaHastaSQL;
    }
    $stmtResultadoPerfilHistorico->execute($paramsResultadoPerfil);
    $rowsResultadoPerfilHistorico = $stmtResultadoPerfilHistorico->fetchAll(PDO::FETCH_ASSOC);

    // Organizar datos adicionales por F9CallID
    $datosAdicionalesPorF9CallID = [];
    foreach ($rowsResultadoPerfilHistorico as $row) {
        if (!isset($datosAdicionalesPorF9CallID[$row['F9CallID']])) {
            $datosAdicionalesPorF9CallID[$row['F9CallID']] = [];
        }
        $datosAdicionalesPorF9CallID[$row['F9CallID']][$row['clave']] = $row['valor'];
    }

    // Obtener datos de avi_call_costs (duración y costo LLM)
    $sqlCostosHistorico = "
        SELECT f9_call_id, connection_duration_secs, llm_cost_total_usd
        FROM avi_call_costs
        WHERE cliente = :cliente
    ";
    ...
    // Agregar filtros de fecha si existen
    if (!empty($fechaDesdeSQL) && !empty($fechaHastaSQL)) {
        $sqlCostosHistorico .= " AND DATE(metadata_date_local) BETWEEN :fecha_desde AND :fecha_hasta";
    }

    $stmtCostosHistorico = $pdo->prepare($sqlCostosHistorico);
    $paramsCostos = [':cliente' => $clienteActual];
    if (!empty($fechaDesdeSQL) && !empty($fechaHastaSQL)) {
        $paramsCostos[':fecha_desde'] = $fechaDesdeSQL;
        $paramsCostos[':fecha_hasta'] = $fechaHastaSQL;
    }
    $stmtCostosHistorico->execute($paramsCostos);
    $rowsCostosHistorico = $stmtCostosHistorico->fetchAll(PDO::FETCH_ASSOC);

    // Organizar costos por F9CallID
    $costosPorF9CallID = [];
    foreach ($rowsCostosHistorico as $row) {
        $costosPorF9CallID[$row['f9_call_id']] = [
            'duracion' => $row['connection_duration_secs'],
            'costo_llm' => $row['llm_cost_total_usd']
        ];
    }
...
<body class="bg-gray-50 min-h-screen">
        ...
        <section class="mb-10" id="realtime-section">
            ...
            <div class="relative mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <!-- LLM Costo (Aproximado) -->
                    <div class="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-lg shadow-md p-6 text-white">
                        <div class="flex items-center justify-between mb-2">
                            <h3 class="text-sm font-semibold opacity-90">LLM Costo (Aproximado)</h3>
                            <i class="fas fa-dollar-sign text-2xl opacity-50"></i>
                        </div>
                        <p class="text-4xl font-bold"><span id="rt-llm-costo">$<?= number_format($llmCostoTotalHoy, 2) ?></span></p>
                        <p class="text-xs opacity-75 mt-1">USD total</p>
                    </div>

                    <!-- LLM Costo Promedio -->
                    <div class="bg-gradient-to-br from-cyan-500 to-cyan-600 rounded-lg shadow-md p-6 text-white">
                        <div class="flex items-center justify-between mb-2">
                            <h3 class="text-sm font-semibold opacity-90">LLM Costo Promedio</h3>
                            <i class="fas fa-calculator text-2xl opacity-50"></i>
                        </div>
                        <p class="text-4xl font-bold"><span id="rt-llm-costo-promedio">$<?= number_format($llmCostoPromedioHoy, 3) ?></span></p>
                        <p class="text-xs opacity-75 mt-1">USD por llamada</p>
                    </div>
            ...
            <?php if ($aplicarFiltros): ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                    ...
                    <div class="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-lg shadow-md p-6 text-white">
                        ...
                        <p class="text-4xl font-bold">$<?= number_format($llmCostoTotalHistorico, 2) ?></p>
                        <p class="text-xs opacity-75 mt-1">USD total</p>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-cyan-500 to-cyan-600 rounded-lg shadow-md p-6 text-white">
                        ...
                        <p class="text-4xl font-bold">$<?= number_format($llmCostoPromedioHistorico, 3) ?></p>
                        <p class="text-xs opacity-75 mt-1">USD por llamada</p>
                    </div>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-purple-50 transition">
                                    ...
                                    <td class="px-4 py-3 text-right font-semibold text-purple-700">
                                        <?php
                                        $retencionExitosa = ($resultados['contencion_exitosa'] ?? 0) + ($resultados['consulta_resuelta'] ?? 0) + ($resultados['consulta_resuelta_pase_asesor'] ?? 0);
                                        $porcentajeRetencion = $avi > 0 ? ($retencionExitosa / $avi) * 100 : 0;
                                    ...
                                    <?php foreach ($todosResultadosHistorico as $resultado): ?>
                                        ...
                                        <td class="px-4 py-3 text-right text-gray-600">
                                            <?= isset($resultados[$resultado]) && $avi > 0 ? number_format(($resultados[$resultado] / $avi) * 100, 1) . '%' : '-' ?>
                                        </td>
                                    <?php endforeach; ?>
                                    ...
                                    <td class="px-4 py-3 text-right text-gray-600">
                                        <?= $five9 > 0 ? number_format(($colgadas / $five9) * 100, 1) . '%' : '-' ?>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                ...
                                <tr class="bg-purple-100 border-t-2 border-purple-600 font-bold text-base">
                                    ...
                                    <td class="px-4 py-4 text-right text-purple-800">
                                        <?php
                                        $retencionTotal = ($totalesPorResultadoHistorico['contencion_exitosa'] ?? 0) + ($totalesPorResultadoHistorico['consulta_resuelta'] ?? 0) + ($totalesPorResultadoHistorico['consulta_resuelta_pase_asesor'] ?? 0);
                                        $porcentajeRetencionTotal = $totalAVIGeneral > 0 ? ($retencionTotal / $totalAVIGeneral) * 100 : 0;
                                    ...
                                    <?php foreach ($todosResultadosHistorico as $resultado): ?>
                                        ...
                                        <td class="px-4 py-4 text-right text-purple-700">
                                            <?= $totalAVIGeneral > 0 ? number_format(($totalesPorResultadoHistorico[$resultado] / $totalAVIGeneral) * 100, 1) . '%' : '-' ?>
                                        </td>
                                    <?php endforeach; ?>
                                    <td class="px-4 py-4 text-right text-purple-800">
                                        <?= number_format($totalColgadasGeneral) ?>
                                    </td>
                                    <td class="px-4 py-4 text-right text-purple-700">
                                        <?= $totalFive9General > 0 ? number_format(($totalColgadasGeneral / $totalFive9General) * 100, 1) . '%' : '-' ?>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                            ...
                            <table class="w-full text-sm" id="tablaInteracciones">
                                <thead class="bg-gradient-to-r from-purple-600 to-purple-700 text-white">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-right font-bold">Costo LLM</th>
                                    </tr>
                                </thead>
                                <tbody id="interaccionesContainer" class="divide-y divide-gray-200">
                                    <?php $count = 0; foreach ($leadsHistorico as $f9id => $values): $count++;
                                        // Obtener datos adicionales de ResultadoPerfil
                                        $resultado = $datosAdicionalesPorF9CallID[$f9id]['resultado_llamada'] ?? '-';
                                        // Obtener datos de avi_call_costs
                                        $duracion = $costosPorF9CallID[$f9id]['duracion'] ?? 0;
                                        $costoLLM = $costosPorF9CallID[$f9id]['costo_llm'] ?? 0;
                                    ?>
                                        <tr class="interaccion-item hover:bg-purple-50 transition"
                                            ...
                                            <td class="px-4 py-3 text-right text-gray-700 text-xs">$<?= number_format($costoLLM, 3) ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
    ...
    <script>
        ...
        function updateRealtimeUI(data) {
            ...
            // Actualizar LLM Costo
            const llmCostoElement = document.getElementById('rt-llm-costo');
            if (llmCostoElement) {
                llmCostoElement.textContent = '$' + data.llmCosto.toFixed(2);
            }

            // Actualizar LLM Costo Promedio
            const llmCostoPromedioElement = document.getElementById('rt-llm-costo-promedio');
            if (llmCostoPromedioElement) {
                llmCostoPromedioElement.textContent = '$' + data.llmCostoPromedio.toFixed(3);
            }
        ...
        function openDetailModal(f9CallID) {
            ...
            iframe.src = 'detalle_lead_v2_optimizado.php?F9CallID=' + encodeURIComponent(f9CallID);
        ...
        function exportResumenHistoricoToExcel() {
            ...
            // Patrón: Fecha(0), Five9(1), AVI(2), %Ret(3), Result1(4), %(5), Result2(6), %(7), ...
            for (let C = 0; C <= range.e.c; C++) {
                // Columnas de porcentaje: índice 3 (% Retención) y columnas impares después de 4
                if (C === 3 || (C > 4 && C % 2 === 1)) {
        ...
        function exportarInteraccionesExcel() {
            ...
            // Encabezados
            data.push(['F9CallID', 'F9TimeStamp', 'ANI', 'Resultado', 'Duración', 'Costo LLM']);
