...
// ============================================
// OPTIMIZACIÓN: CONSOLIDAR QUERIES EN UNO SOLO
// ============================================

// Query único optimizado para obtener todos los datos de siigo_lead_data_v2
$stmt_datos = $pdo->prepare("
    SELECT
        ...
        method,
        CASE
            WHEN clave = 'F911DNIS' AND method = 'Insert Call' THEN 1
            ELSE 0
        END as is_f911dnis
...
");
$stmt_datos->execute([':f9id' => $f9id]);
$allDatos = $stmt_datos->fetchAll(PDO::FETCH_ASSOC);

// Separar datos por método en PHP (más rápido que múltiples queries)
$insertCall = [];
$resultadoPerfil = [];
$f911dnis = null;

foreach ($allDatos as $row) {
    if ($row['method'] === 'Insert Call') {
        $insertCall[] = ['clave' => $row['clave'], 'valor' => $row['valor']];
        if ($row['is_f911dnis']) {
            $f911dnis = $row['valor'];
        }
    } elseif ($row['method'] === 'ResultadoPerfil') {
        $resultadoPerfil[] = ['clave' => $row['clave'], 'valor' => $row['valor']];
    }
}
...
if (!empty($f911dnis)) {
    ...
    $stmt_transcripciones = $pdo->prepare("
        SELECT
            ...
            created_at
        FROM eleven_n8n_t1
        WHERE from_number = :dnis
        ORDER BY created_at DESC
        LIMIT 50
    ");
    $stmt_transcripciones->execute([':dnis' => $f911dnis]);
    $transcripciones = $stmt_transcripciones->fetchAll(PDO::FETCH_ASSOC);

    // Obtener ElevenConversationIDs
    if (!empty($transcripciones)) {
        ...
        if (!empty($conversationIDs)) {
            ...
            $stmt_analisis = $pdo->prepare("
                SELECT
                    ElevenConversationID,
                    criteria_id,
                    result,
                    rationale
                FROM eleven_n8n_t1_analisis
                WHERE ElevenConversationID IN ($placeholders)
                LIMIT 500
            ");
            $stmt_analisis->execute($conversationIDs);
...
<body class="bg-gray-50">
    <div class="min-h-screen">
        ...
        <main class="max-w-7xl mx-auto px-4 py-8">
            ...
            <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                <h2 class="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-2">
                    <i class="fas fa-microphone text-blue-600"></i>Transcripciones (<?php echo count($transcripciones); ?>)
                </h2>
                <div class="space-y-4">
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                <div class="p-6 border-b border-gray-200">
                    <h2 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
                        <i class="fas fa-table text-blue-600"></i>Análisis Detallado (<?php echo count($analisis); ?> criterios)
                    </h2>
                </div>
