...
NEQUI Dashboard is a PHP-based call tracking and lead management system that displays call analytics, transcriptions, detailed lead information, and concurrency reports for multiple clients. The application supports multi-client configuration and provides multiple views for monitoring call campaigns.

## Multi-Client Configuration

The dashboard supports multiple clients through a centralized configuration system.

### Main config (`config.php`)
- Used by all standard dashboard pages (dashboard_consolidado.php, detalle_lead_v2.php, etc.)
- Defines `CLIENTE_ACTUAL` constant (currently `'NEQUI2'`)
- Provides `getDBConnection()` helper function
- Clients: NEQUI, NEQUI2 (default), ICQ24-GQA

### Report config (`config_reporte_concurrencia.php`)
- Used exclusively by `reporte_concurrencia.php`
- Same DB credentials as `config.php`
- Includes `CLIENTES_DISPONIBLES` catalog and `buildClienteCondition()` helper
- Supports an `'ALL'` pseudo-client that combines NEQUI + NEQUI2 data

### Client Selector Pattern
Pages that support client switching use a combobox in the header:
```php
...
```
The combobox form uses `onchange="this.form.submit()"` and preserves other filter params via hidden inputs.

All navigation (filters, "Limpiar", auto-refresh API calls, detail modals) must propagate the `cliente` parameter:
- Form hidden inputs: `<input type="hidden" name="cliente" value="...">`
- Auto-refresh fetch: `dashboard_realtime_data.php?cliente=...`
- Detail iframe: `detalle_lead_v2.php?F9CallID=...&cliente=...`
...
- Database: `n8n_icq`
- Used by: All main dashboard files + reportes
- Connection via `getDBConnection()` from `config.php` or `config_reporte_concurrencia.php`

**Important:** Database credentials are centralized in config files. When modifying database-related code, always use `require_once 'config.php'` (or `config_reporte_concurrencia.php`) and `getDBConnection()` instead of creating new PDO connections.
...
- **level_calls**: Call tracking table with columns `F9CallID`, `F9TimeStamp`, `PROYECTO`, `ANI`, `DNIS`, `cliente`, `Estado`, `duration`, `ElevenTimeStamp`. Used by concurrency report.
...
16. **reporte_concurrencia.php** - Concurrency report page
    - Displays concurrent call analysis per day using `level_calls` table
    - Date range filter with client selector combobox
    - KPIs: days consulted, total calls, max concurrent (peak)
    - Chart.js dual-axis chart: bar (total calls) + line (max concurrent) with independent Y scales
    - Table: date, total calls, avg duration (min), max concurrent, utilization factor
    - Uses `config_reporte_concurrencia.php` for configuration
    - **Concurrency calculation is done client-side in JavaScript** to avoid heavy SQL window functions:

### Concurrency Algorithm (JavaScript)

The calculation follows an **event-based approach** entirely in the browser:

1. **Fetch raw data**: Simple SQL query returns only `F9CallID, F9TimeStamp, duration, ElevenTimeStamp` per call
2. **Build events**: Each call generates two events — `+1` at start time (`F9TimeStamp`), `-1` at end time (`ElevenTimeStamp` or `F9TimeStamp + duration`)
3. **Sort & accumulate**: Events are grouped by day (`getDateKey`), sorted by timestamp, and a running sum tracks active calls at each point
4. **Max per day**: The highest running sum value for each day = `max_concurrent`

```javascript
const eventos = calls.flatMap(call => {
    const start = parseDate(call.F9TimeStamp);
    const end = getEndTime(call); // ElevenTimeStamp or F9TimeStamp + duration
    return [
        { fecha: getDateKey(start), ts: start.getTime(), delta: +1 },
        { fecha: getDateKey(end),   ts: end.getTime(),   delta: -1 }
    ];
});
// Group by day, sort by time, running sum, track max
```

This approach keeps the SQL lightweight and moves processing to the client.

## Helper: `buildClienteCondition()`

Defined in `config_reporte_concurrencia.php`. Generates SQL condition and PDO parameters for filtering by client.

```php
function buildClienteCondition($clienteActual, $alias = '')
```

- If `$clienteActual === 'ALL'`: returns `cliente IN (:c_0, :c_1)` with values from `CLIENTES_DISPONIBLES` (excluding 'ALL')
- Otherwise: returns `cliente = :cliente` with the single value
- `$alias` parameter adds a table alias prefix (e.g., `'lc'` → `lc.cliente = :cliente`)

Returns: `['sql' => '...', 'params' => [':cliente' => '...']]`
...
$stmt = $pdo->prepare("SELECT ... WHERE {$cc['sql']} AND DATE(F9TimeStamp) BETWEEN ...");
$stmt->execute($params);
```

## Client Propagation Rules

When adding features that support multiple clients:
1. All URL links must pass `&cliente=...` or use hidden form inputs
2. `dashboard_realtime_data.php` — reads `$_GET['cliente']` with validation
3. `detalle_lead_v2.php` — reads `$_GET['cliente']` with fallback
4. Detail modal iframe URL must include `&cliente=...`
5. "Limpiar" reset link must preserve `?cliente=...`
6. Auto-refresh fetch calls must include `?cliente=...`
...
- Main database queries filter by `cliente = :cliente` constant (or via `buildClienteCondition()`)
- For multi-client queries, use `buildClienteCondition($clienteActual, $alias)` helper
...
- `level_calls` table: used by `reporte_concurrencia.php` for concurrency analysis (columns: F9CallID, F9TimeStamp, PROYECTO, ANI, DNIS, cliente, Estado, duration, ElevenTimeStamp)
